...
export function SkillChip({ skill }: SkillChipProps) {
  ...
  const tooltipFooter = usedIn.length ? `Used in ${shownContexts}` : undefined;
  ...
  const detailId = `skill-${skill.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
  const detail = `${skill.role}${usedIn.length ? `, used in ${usedIn.join(", ")}` : ""}`;
  ...
  return (
    <StaggerItem className="group/skill relative">
      <span
        ...
        className="group inline-flex items-center gap-2.5 rounded-xl border border-white/6 bg-white/[0.02] px-3 py-2 text-sm text-mist-200 transition-[transform,border-color,background-color,color] duration-200 group-focus-within/skill:border-azure-400/40 group-hover/skill:-translate-y-0.5 group-hover/skill:border-white/15 group-hover/skill:bg-white/[0.05]"
      >
        <TechIcon
          tech={skill.name}
          className="size-4 text-mist-400 group-hover/skill:text-(--brand)"
          brandOnHover={false}
        />
        {skill.name}
      </span>{" "}
