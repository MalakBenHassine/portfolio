      ...
      {results.map((result) => (
        <StaggerItem key={result.label}>
          <SpotlightCard className="flex h-full flex-col p-6">
            {/* Same metric height in every card, so titles and details line up across the row. */}
            <p className="text-4xl font-semibold tracking-[-0.04em] sm:min-h-[7.25rem] sm:text-[2.6rem]">
              {result.kind === "before-after" ? (
                ...
                <span className="text-gradient">
                  <AnimatedCounter value={result.value ?? 0} suffix={result.suffix} pad={result.pad} />
                </span>
              )}
            </p>
            <p className="pt-5 font-medium text-snow">{result.label}</p>
